...
public class PlayerDAO {
            ...
            try {
                ...
                if (player.isDie()) {
                    ...
                    if (MapService.gI().isMapDoanhTrai(mapId)  || MapService.gI().isMapBlackBallWar(mapId)|| MapService.gI().isMapDiaNguc(mapId)
                         ||MapService.gI().isMapConDuongRanDoc(mapId) || MapService.gI().isMapKhiGaHuyDiet(mapId) || MapService.gI().isMapBanDoKhoBau(mapId) || MapService.gI().isMapMaBu(mapId)) {
                        mapId = player.gender + 21;
                        x = 300;
                        y = 336;
                    }
                ...
                String query = " update player set info_achievement =?,data_item_time_sieu_cap = ?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, tongtien=?, flagMoc100k = ?, flagMoc200k = ?, flagMoc500k = ?, flagMoc1cu = ?, pointPvp=?,data_card=?,bill_data =? where id = ?";
    ...
    public static boolean subcoinBar1(Player player, int num) {
        PreparedStatement ps = null;
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("update account set coin = (coin - ?), is_admin = ? where id = ?");
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().bg ? 1 : 0);
            ps.setInt(3, player.getSession().userId);
            ps.executeUpdate();
            ps.close();
            player.getSession().coinBar -= num;
