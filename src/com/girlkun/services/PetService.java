...
public class PetService {
    ...
    public void createzenoPet(Player player, int gender, byte... limitPower) {
        ...
        public void createNKPet(Player player, byte... limitPower) {
        new Thread(() -> {
            try {
    ...
    public void createNKPet(Player player, int gender, byte... limitPower) {
        new Thread(() -> {
            try {
    ...
    public void createSTPet(Player player, byte... limitPower) {
        new Thread(() -> {
            try {
    ...
    public void createSTPet(Player player, int gender, byte... limitPower) {
        new Thread(() -> {
            try {
    ...
    public void changeSTPet(Player player) {
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
        ...
        player.pet.dispose();
        player.pet = null;
        createSTPet(player, limitPower);
    }

    public void changeSTPet(Player player, int gender) {
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
        ...
        player.pet.dispose();
        player.pet = null;
        createSTPet(player, gender, limitPower);
    }
    public void changeNKPet(Player player) {
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
        ...
        player.pet.dispose();
        player.pet = null;
        createNKPet(player, limitPower);
    }

    public void changeNKPet(Player player, int gender) {
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
        ...
        player.pet.dispose();
        player.pet = null;
        createNKPet(player, gender, limitPower);
    }
    ...
    private int[] getDataPetNormal() {
        ...
        petData[3] = Util.nextInt(109, 150); //def
    ...
    private int[] getDataPetMabu() {
     ...
     private int[] getDataPetZeno() {
        ...
        petData[0] = Util.nextInt(40, 115) * 20; //hp
        petData[1] = Util.nextInt(40, 115) * 20; //mp
        petData[2] = Util.nextInt(200, 300); //dame
        petData[3] = Util.nextInt(120, 180); //def
        petData[4] = Util.nextInt(0, 5); //crit
        return petData;
    }
    private void createNewPet(Player player, boolean isMabu, boolean isBerus,boolean isPic,boolean isNK,boolean isST, boolean isZX,byte... gender) {
        int[] data = isMabu ? isPic ? isNK ? isST ? isZX ?getDataPetMabu() : getDataPetPic() :getDataPetNK(): getDataPetST() : getDataPetZeno(): getDataPetNormal();
        Pet pet = new Pet(player);
        pet.name = "$" + (isMabu ? "uub" : isBerus ? "Berus" : isPic ? "Ngộ Không" : isNK ? "Bát Giới" : isST ? "Sa Tăng" : isZX ? "Zeno Sama":"Đệ tử");
        pet.gender = (gender != null && gender.length != 0) ? gender[0] : (byte) Util.nextInt(0, 2);
        pet.id = -player.id;
        pet.nPoint.power = isMabu || isBerus|| isPic || isNK || isST || isZX? 1500000 : 2000;
        pet.typePet = (byte) (isMabu ? 1 : isBerus ? 2 :isPic ? 3 :isNK ? 4 :isST ? 5 :isZX ? 6: 0);
