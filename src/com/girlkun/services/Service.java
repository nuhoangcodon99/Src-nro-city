...
//                }
            ...
            if (text.equals("ad")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1,
                        "|7| - Ngọc Rồng City - " + "\n"
                        + "|4|Sessions: " + Client.gI().getPlayers().size() + "\n"
                        + "|5|Tổng thread: " + Thread.activeCount() + "\n"
                        + "|6|Số Người Đang Online : " + Client.gI().getPlayers().size() + "\n"
                        + "|8|Thời Gian Chạy Server : " + ServerManager.timeStart + "\n"
                        + "|3|[ VUI LÒNG CHỌN QUYỀN HẠN ]\n",
                        "Bảo Trì", "Quản Lý\nAccount", "Buff\nItem", "Menu\nBoss", "Đóng");
                return;
...
//            Send_Info_NV(pl);
    ...
    public void Send_Caitrang(Player player) {
        if (player != null) {
            ...
            try {
                ...
                msg.writer().writeByte(player.effectSkill.isBienHinh ? 1 : 0);//set biến hình
                sendMessAllPlayerInMap(player, msg);
                msg.cleanup();
            } catch (Exception e) {
                Logger.logException(Service.class, e);
    ...
    public static final int[] flagTempId = {363, 364, 365, 366, 367, 368, 369, 370, 747, 519, 520, 371};
    public static final int[] flagIconId = {2761, 2330, 2323, 2327, 2326, 2324, 2329, 2328, 2325, 4386, 4385, 2331};
...
//            System.out.println(platform);
    ...
    public void DropVeTinh(Player pl, Item item, Zone map, int x, int y) {
        ItemMap itemMap = new ItemMap(map, item.template, item.quantity, x, y, 0);
