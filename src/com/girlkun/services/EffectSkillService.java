...
//        player.effectSkill.timeAnTroi = timeAnTroi;
    ...
    public void removeHuytSao(Player player) {
        player.effectSkill.tiLeHPHuytSao = 1;
   ...
   public void setBienHinh(Player player) {
        ...
        Skill template = Manager.NCLASS.get(player.gender)
                .getSkillTemplate(player.playerSkill.skillSelect.template.id)
                .skillss.stream().filter(s -> s.point == player.playerSkill.skillSelect.point)
     ...
     Service.getInstance().player(player);
        if (!lastLevel && template.coolDown > 0) {
            player.playerSkill.skillSelect.coolDown = template.coolDown * 1 / 900;
        } else {
