...
public class Input {
    ...
    public static final int CHOOSE_LEVEL_BTDN = 516;
 ...
 public static final int UseGold = 3;
            ...
            switch (player.iDMark.getTypeInput()) {
               ...
               case GIFT_CODE:{
                ...
                break;
                 case TAI_taixiu:
                    ...
                    try {
                        if (sotvxiu1 >= 100000 && sotvxiu1 <= 100000) {
                            ...
                            Service.gI().sendThongBao(player, "Cược Mặc Định 100K Hồng ngọc");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    ...
                    try {
                        if (sotvxiu2 >= 100000 && sotvxiu2 <= 100000) {
                    ...
                    Item tv1 = null;
                        ...
                        if (tvCuoc1 >= 100 && tvCuoc1 <= 100) {
                            ...
                            Service.gI().sendThongBao(player, "Cược Mặc Định 100 Thỏi vàng");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                        Service.gI().sendThongBao(player, "Lỗi.");
                ...
                case LE:
                    ...
                    Item tv = null;
                        ...
                        if (tvCuoc >= 100 && tvCuoc <= 100) {
                            ...
                            Service.gI().sendThongBao(player, "Thỏi vàng");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                        Service.gI().sendThongBao(player, "Lỗi.");
                    ...
                    case CHOOSE_LEVEL_BTDN:
                    int level5 = Integer.parseInt(text[0]);
                    if (level5 >= 1 && level5 <= 110) {
                        Npc npc = NpcManager.getByIdAndMap(ConstNpc.MiNuong, player.zone.map.mapId);
                        if (npc != null) {
                            npc.createOtherMenu(player, ConstNpc.MENU_ACCEPT_GO_TO_BTDN,
                                    "Con có chắc chắn muốn tới khí gas hủy diệt cấp độ " + level5 + "?",
                                    new String[]{"Đồng ý", "Từ chối"}, level5);
                        }
                    } else {
                        Service.gI().sendThongBao(player, "Không thể thực hiện");
                    }                    
               ...
               case CHOOSE_LEVEL_BDKB:
                ...
                case QUY_DOI_COIN:
                    ...
                    int coinGold = 1000; // chia 1000
                    int goldTrade = Integer.parseInt(text[0]);
                    if(goldTrade<=0 || goldTrade>= 1000)
                    {
                       Service.gI().sendThongBao(player, "Quá giới hạn mỗi lần chỉ được 1M");
                    }
    ...
    public void createFormUseGold(Player pl) {
        createForm(pl, UseGold, "Max 1000Tv Hơn Mất", new SubInput("1 thỏi vàng được 500tr vàng max 500 tỷ", NUMERIC));
    }
    ...
    public void createFormQDTV(Player pl) {
        ...
        createForm(pl, QUY_DOI_COIN, "Quy đổi thỏi vàng, 10.000 coin = 40 tv "
              , new SubInput("Nhập số coin muốn đổi (bỏ 3 số 0)", NUMERIC));
    }
    public void createFormQDN(Player pl) {
      
        createForm(pl, QUY_DOI_NGOC, "Quy đổi hồng ngọc, 10.000 coin = 40.000 hn "
              , new SubInput("Nhập số coin muốn đổi (bỏ 3 số 0)", NUMERIC));
    }
    ...
    public void createFormChooseLevelBTDN(Player pl) {
        createForm(pl, CHOOSE_LEVEL_BTDN, "Chọn cấp độ", new SubInput("Cấp độ (1-110)", NUMERIC));
    }
