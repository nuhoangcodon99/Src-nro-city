...
public class ChangeMapService {
    ...
    public void openZoneUI(Player pl) {
        ...
        if (!pl.isAdmin()) {
            ...
            if (MapService.gI().isMapBayTangDiaNguc(pl.zone.map.mapId)) {
                Service.getInstance().sendThongBaoOK(pl, "Không thể đổi khu vực trong map này");
                return;
            }
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (pl.isAdmin() || pl.isBoss || Util.canDoWithTime(pl.iDMark.getLastTimeChangeZone(), 4000)) {
            ...
            if (zoneId >= 0 && zoneId <= map.zones.size() - 1) {
                ...
                if (zoneJoin != null) {
                    if (!pl.isBoss) {
                        zoneJoin.hours = Calendar.getInstance().get(Calendar.HOUR_OF_DAY);
                        if (zoneJoin.hours >= ConstMap.TIME_START_SUPPORT && zoneJoin.hours <= ConstMap.TIME_END_SUPPORT) {
                            if (!zoneJoin.getBosses().isEmpty()) {
                                List<Player> listBoss = zoneJoin.getBosses();
                                for (Player plBoss : listBoss) {
                                    if (plBoss != null) {
                                        if (pl.playerTask.taskMain.id != 19 && (plBoss.id == BossID.KUKU || plBoss.id == BossID.MAP_DAU_DINH || plBoss.id == BossID.RAMBO)) {

                                            Service.getInstance().sendThongBaoOK(pl, "  Trong thời gian hỗ trợ\nKhông thể vào khu vực này");
                                            return;
						   }
                                        if (pl.playerTask.taskMain.id != 20 && (plBoss.id == BossID.TDST)) {

                                            Service.getInstance().sendThongBaoOK(pl, "  Trong thời gian hỗ trợ\nKhông thể vào khu vực này");
                                            return;
                                        }
                                        if (pl.playerTask.taskMain.id != 21 && (plBoss.id == BossID.FIDE)) {
            ...
            Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực lúc này, vui lòng đợi "
                    + TimeUtil.getTimeLeft(pl.iDMark.getLastTimeChangeZone(), 4));
        }
    }
    ...
    public void finishLoadMap(Player player) {
        ...
        Logger.log(Logger.CYAN, "Cư Dân "  + player.name +  " đang ở " + player.zone.map.mapName + " khu " + player.zone.zoneId + "\n");
    }
...
//        }
            ChangeMapService.gI().changeMapBySpaceShip(player, 135, -1, 36);
    }
    
    public void goToBTDN(Player player) {
//        if (!player.iDMark.isGoToKGHD()) {
//            player.iDMark.setLastTimeGoToKGHD(System.currentTimeMillis());
//            player.iDMark.setGoToKGHD(true);
//            spaceShipArrive(player, (byte) 1, DEFAULT_SPACE_SHIP);
//            effectChangeMap(player, 10, EFFECT_GO_TO_BDKB);
////            player.type = 4;
////            player.maxTime =5;
//        }
            ChangeMapService.gI().changeMapBySpaceShip(player, 180, -1, 174);
...
////            player.maxTime =5;
//        }
            ChangeMapService.gI().changeMapBySpaceShip(player, 149, -1, 100);
...
////            player.maxTime = 6;
//        }
        ChangeMapService.gI().changeMapBySpaceShip(player, 143, -1, 1108);
    }
...
//        //dong map 
        ...
        if (zoneJoin != null) {
            switch (zoneJoin.map.mapId) {
                ...
                case 68: //thung lũng nappa
                    ...
                    break;
                case 69: //vực cấm
                    if (TaskService.gI().getIdTask(player) < ConstTask.TASK_18_1) {
                        Service.gI().sendThongBao(player, "Vui lòng hoàn thành nhiệm vụ trước khi tới đây!");
                        return null;
                    }
                    break;
                ...
                case 71: //căn cứ rasphery
                    if (TaskService.gI().getIdTask(player) < ConstTask.TASK_18_3) {
                        Service.gI().sendThongBao(player, "Vui lòng hoàn thành nhiệm vụ trước khi tới đây!");
                        return null;
                    }
                    break;
                case 72: //thung lũng rasphery
                    ...
                    break;
                case 64: //núi dây leo
                    if (TaskService.gI().getIdTask(player) < ConstTask.TASK_18_5) {
                        Service.gI().sendThongBao(player, "Vui lòng hoàn thành nhiệm vụ trước khi tới đây!");
                        return null;
                    }
                    break;
                ...
                case 80: //núi khỉ vàng
                    ...
                    break;
                case 104: //núi khỉ vàng qua cold
                ...
                case 100: //thị trấn ginder
                    ...
                    break;
                case 103: //võ đài xên
                    if (TaskService.gI().getIdTask(player) < ConstTask.TASK_27_3) {
                        Service.gI().sendThongBao(player, "Vui lòng hoàn thành nhiệm vụ trước khi tới đây!");
                        return null;
                    }
                    break;
                    ...
                    case 146:
                    case 147:    
                        if (TaskService.gI().getIdTask(player) < ConstTask.TASK_28_1) {
                        Service.gI().sendThongBao(player, "Vui lòng hoàn thành nhiệm vụ trước khi tới đây!");
                        return null;
                    }
                    break;
    ...
    public void changeMapBaHatMit(Player pkbahatmit, int i, int i0, int i1, int i2) {
