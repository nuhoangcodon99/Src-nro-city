...
import com.girlkun.models.map.BayTangDiaNguc.BayTangDiaNgucService;
        ...
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (this.mapId == 211 || this.mapId == 5) {
                        ...
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                ...
                                case 1:
                                        ...
                                        Service.gI().sendThongBao(player, "Ngươi Phải Hoàn Thành Nhiệm Vụ 26 Trước Khi Đến Đây.");
                                        return;
                                    } else {
                                        ChangeMapService.gI().changeMapBySpaceShip(player, 212, -1, 149);
                                        break;
                                    }
                            }
...
public static Npc gapthu(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        ...
        public void openBaseMenu(Player player) {
            if (canOpenNpc(player)) {
                if (this.mapId == 5) {
                    this.createOtherMenu(player, 1234, "|0|Ngọc Rồng City\n"+"|2|MÁY GẮP TRỨNG\n"+"|1|GẮP X1 : 5000 Hồng Ngoc\nGẮP X10 : 50000 Hồng Ngoc\nGẮP X100 : 500000 Hồng Ngoc\n"+"|7|LƯU Ý : Nếu hành trang đầy sẽ chuyển trứng về ruơng phụ\nNẾU MUỐN NGƯNG AUTO GẤP CHỈ CẦN THOÁT GAME VÀ VÀO LẠI!",
                        ...
                        switch (select) {
                            ...
                            case 2:
                                if (player.inventory.ruby < 500000) {
                                    Service.gI().sendThongBao(player, "không đủ 500000 Hồng Ngoc");
                                    return;
                                }
                                try {
                ...
                if (canOpenNpc(player)) {
                    if (player.clan == null) {
                        this.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Chỉ tiếp các bang hội, miễn người không vào bang", "Đóng");
                        return;
                    }
            ...
            public void confirmMenu(Player player, int select) {
                if (canOpenNpc(player)) {
                    if (player.iDMark.isBaseMenu()) {
                        ...
                        switch (select) {
                            case 0:
                                if (player.isAdmin() && player.clanMember.getNumDateFromJoinTimeToToday() >= 1 || player.nPoint.power >= KhiGasHuyDiet.POWER_CAN_GO_TO_KGHD && player.clanMember.getNumDateFromJoinTimeToToday() >= 1) {
...
private static Npc Minuong(int mapId, int status, int cx, int cy, int tempId, int avartar) {
         return new Npc(mapId, status, cx, cy, tempId, avartar) {
            @Override
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
...
//                            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con, con muốn ta giúp gì nào?", "Giải tán bang hội", "Nhận quà\nđền bù");
                        ...
                        this.createOtherMenu(player, ConstNpc.BASE_MENU, "Đây Là Phó Bản Mị Nương\nỞ Đây Có Ta Lo\nCác Ngươi Cứ Yên Tâm Lên Đường.\nTa sẽ đưa các cậu đến nơi ấy, các cậu sẵn sàng chưa?", "Thông tin chi tiết", "Top 100 bang hội", "OK", "Từ chối");
                    }
                }
            }
            ...
            public void confirmMenu(Player player, int select) {
                if (canOpenNpc(player)) {
                        ...
                        switch (select) {
                            ...
                            case 2:
                              if (!player.getSession().actived){
                                    ...
                                    if (player.clan.BayTangDiaNguc != null) {
                                        this.createOtherMenu(player, ConstNpc.MENU_OPENED_BTDN,
                                                "Bang hội của con đang đi phó bản mị nương cấp độ "
                                                + player.clan.BayTangDiaNguc.level + "\nCon có muốn đi theo không?",
                                                "Đồng ý", "Từ chối");
                                    } else {

                                        this.createOtherMenu(player, ConstNpc.MENU_OPEN_BTDN,
                                                "Đây là phó bản mị nương\nCác con cứ yên tâm lên đường\n"
                                                + "Ở đây có ta lo\nNhớ chọn cấp độ vừa sức mình nhé",
                                                "Chọn\ncấp độ", "Từ chối");
                                    }
                                } else {
                    ...
                    } else if (player.iDMark.getIndexMenu() == ConstNpc.MENU_OPEN_BTDN) {
                        switch (select) {
                            case 0:
                                if (player.isAdmin() && player.clanMember.getNumDateFromJoinTimeToToday() >= 1 || player.nPoint.power >= BayTangDiaNguc.POWER_CAN_GO_TO_BTDN && player.clanMember.getNumDateFromJoinTimeToToday() >= 1) {
                                    Input.gI().createFormChooseLevelBTDN(player);
                                }if (player.clanMember.getNumDateFromJoinTimeToToday() < 1) {
                                    Service.gI().sendThongBao(player, "Yêu cầu tham gia bang hội trên 1 ngày!");
                                    }  else if (player.clan.haveGoneBayTangDiaNguc) {
                                        createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                                "Bang hội của ngươi đã đi con đường rắn độc lúc " + TimeUtil.formatTime(player.clan.lastTimeOpenBayTangDiaNguc, "HH:mm:ss") + " hôm nay. Người mở\n"
                                                + "(" + player.clan.doanhTrai_playerOpen + "). Hẹn ngươi quay lại vào ngày mai", "OK", "Hướng\ndẫn\nthêm");
                                        return;
                    ...
                    } else if (player.iDMark.getIndexMenu() == ConstNpc.MENU_ACCEPT_GO_TO_BTDN) {
                        switch (select) {
                            case 0:
                                BayTangDiaNgucService.gI().openBayTangDiaNguc(player, Byte.parseByte(String.valueOf(PLAYERID_OBJECT.get(player.id))));
                                break;
                        }
                    }
                }
...
public static Npc meothantai1(int mapId, int status, int cx, int cy, int tempId, int avartar) {
    ...
    public static Npc khidaumoi(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            @Override
            public void openBaseMenu(Player player) {
                if (this.mapId == 14 || this.mapId == 80) {
                    this.createOtherMenu(player, ConstNpc.BASE_MENU,
                            ...
                            switch (select) {
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < 12; i++) {
                                        ...
                                        if (InventoryServiceNew.gI().isExistItemBag(player, 1136 + i) && soLuong >= 30 && soLuongg >= 1 && soLuonggg >= 30) {
                                            ...
                                            this.npcChat(player, "Yêu cầu cần cải trang khỉ cấp 2 với 30 đuôi khỉ với 30 thỏi vàng");
                                        }
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < 12; i++) {
                                        ...
                                        if (InventoryServiceNew.gI().isExistItemBag(player, 1137 + i) && soLuong >= 40 && soLuongg >= 1 && soLuonggg >= 80) {
                                            CombineServiceNew.gI().khilv4(player, 1138 + i);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, dns, 40);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, klv, 1);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, tv, 80);
                                            this.npcChat(player, "Upgrade Thành Công!");

                                            break;
                                        } else {
                                            this.npcChat(player, "Yêu cầu cần cải trang khỉ cấp 3 với 40 đuôi khỉ với 80 thỏi vàng");
                                        }
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < 12; i++) {
                                        ...
                                        if (InventoryServiceNew.gI().isExistItemBag(player, 1138 + i) && soLuong >= 70 && soLuongg >= 1 && soLuonggg >= 120) {
                                            CombineServiceNew.gI().khilv5(player, 1112 + i);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, dns, 70);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, klv, 1);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, tv, 120);
                                            this.npcChat(player, "Upgrade Thành Công!");

                                            break;
                                        } else {
                                            this.npcChat(player, "Yêu cầu cần cải trang khỉ cấp 4 với 70 đuôi khỉ với 120 thỏi vàng");
                                        }
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < 12; i++) {
                                        ...
                                        if (InventoryServiceNew.gI().isExistItemBag(player, 1112 + i) && soLuong >= 100 && soLuongg >= 1 && soLuonggg >= 150) {
                                            CombineServiceNew.gI().khilv6(player, 1113 + i);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, dns, 100);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, klv, 1);
                                            ...
                                            this.npcChat(player, "Yêu cầu cần cải trang khỉ cấp 5 với 100 đuôi khỉ với 150 thỏi vàng");
                                        }
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < 12; i++) {
                                        ...
                                        if (InventoryServiceNew.gI().isExistItemBag(player, 1113 + i) && soLuong >= 120 && soLuongg >= 1 && soLuonggg >= 200) {
                                            CombineServiceNew.gI().khilv7(player, 1114 + i);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, dns, 120);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, klv, 1);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, tv, 200);
                                            this.npcChat(player, "Upgrade Thành Công!");

                                            break;
                                        } else {
                                            this.npcChat(player, "Yêu cầu cần cải trang khỉ cấp 6 với 120 đuôi khỉ với 200 thỏi vàng");
                                        }
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < 12; i++) {
                                        ...
                                        if (InventoryServiceNew.gI().isExistItemBag(player, 1114 + i) && soLuong >= 150 && soLuongg >= 1 && soLuonggg >= 300) {
                                            CombineServiceNew.gI().khilv8(player, 1110 + i);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, dns, 150);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, klv, 1);
                                            InventoryServiceNew.gI().subQuantityItemsBag(player, tv, 300);
                                            this.npcChat(player, "Upgrade Thành Công!");

                                            break;
                                        } else {
                                            this.npcChat(player, "Yêu cầu cần cải trang khỉ cấp 7 với 150 đuôi khỉ với 300 thỏi vàng");
                                        }
...
//                            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con, con muốn ta giúp gì nào?", "Giải tán bang hội", "Nhận quà\nđền bù");
                            ...
                            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con, con muốn ta giúp gì nào?","Kho báu dưới biển", "Giải tán bang hội", "Thánh Địa","Next NV","Mốc Nạp");
                        }
                    }
                }
            }
                    ...
                    if (player.iDMark.isBaseMenu()) {
                        switch (select) {
                            ...
                            if (!player.getSession().actived){
                                    ...
                                    this.npcChat(player, "Không có bang");
                                }
                                break;
                            case 1:
                                ...
                                break;
                            case 2:
                                ChangeMapService.gI().changeMapBySpaceShip(player, 159, -1, 432);
                            case 3:
                            ...
                            switch (select) {
                                case 0:
                                    this.createOtherMenu(player, 5,
                                            ...
                                            + "\b|7|Bạn đã nạp tích lũy được: " + Util.format(player.tongtien) + "VND",
                                             "Nhận mốc 100k", "Nhận mốc 200k", "Nhận mốc 500k", "Nhận mốc 1000k");
                                    break;
                            }
                            ...
                            switch (select) {
                                ...
                                case 2:
                                    this.createOtherMenu(player, 88,
                                            "|6|Đây là quà mốc tích lũy!\n"
                                            + "|7|Mốc 500K: Cải trang Zamatsu, Linh Thú, Tuần Lộc\n"
                                            + "|3|Bạn đã nạp tích lũy được: " + Util.format(player.tongtien) + "VND\n"
                                            + "|2|Cảm ơn quý khách đã ghé, chúc bạn chơi game vui vẻ!",
                                            "Nhận ngay", "Từ chối");
                                    break;
                                case 3:
                                    this.createOtherMenu(player, 99,
                                            "|6|Đây là quà mốc tích lũy!\n"
                                            + "|7|Mốc 1000K: 1 Set Kích Hoạt thường, 3 bộ nro\n"
                                            + "|3|Bạn đã nạp tích lũy được: " + Util.format(player.tongtien) + "VND\n"
                                            + "|2|Cảm ơn quý khách đã ghé, chúc bạn chơi game vui vẻ!",
                                            "Nhận ngay", "Từ chối");
                                    break;
                        ...
                        switch (select) {
                            ...
                            case 3:
                                 if (player.inventory.gem == 2000000) {
                                    this.npcChat(player, "Đã Nhận Ngọc Rồi");
                                    break;
                                }
...
//                                } else {
//                                    this.npcChat(player, "Đã Nhận Đệ Tử");
//                                }
//                                break;
                        }
                    } 
    ...
    public static Npc santa(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    createOtherMenu(player, ConstNpc.BASE_MENU,
                            "Xin chào, Chỗ tôi bán đủ hàng cho người dân sever City",
                            "Cửa Hàng","VIP","Support");
                }
            }
                ...
                if (canOpenNpc(player)) {
                    if (this.mapId == 5) {
                        this.createOtherMenu(player, 0,
                                "Chào bạn tôi sẽ đưa bạn đến map đặc biệt?", "Đồng ý", "Từ chối");
                    }
                }
            }
                ...
                if (canOpenNpc(player)) {
                    if (this.mapId == 5) {
                        if (player.iDMark.getIndexMenu() == 0) { // 
                            switch (select) {
                                case 0:
                                    ChangeMapService.gI().changeMapBySpaceShip(player, 211, -1, 979);
...
public static Npc tosukaio(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (this.mapId == 42 || this.mapId == 43 || this.mapId == 44 || this.mapId == 50) {
                        this.createOtherMenu(player, ConstNpc.BASE_MENU,
                                "Ngươi tìm ta có việc gì?","Bông tai\nPorata Cấp 2", "Bông tai\nPorata Cấp 3", "Bông tai\nPorata Cấp 4", "Bông tai\nPorata Cấp 5");
                    }
                }
            }
            ...
            public void confirmMenu(Player player, int select) {
                if (canOpenNpc(player)) {
                    switch (select) {
                        case 0:
                            if (mapId == 0) {
                                if (player.nPoint.power <= 39999999999L) {
                        ...
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                case 0:
                                    ...
                                    Service.gI().sendThongBao(player, "Sức mạnh của bạn không đủ 80 tỷ!");
                                    return;
                                    }
                ...
                if (canOpenNpc(player)) {
                    if (this.mapId == 212) {
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                case 0:
                                    ChangeMapService.gI().changeMapBySpaceShip(player, 5, -1, 1238);
                                    break;                                          
                            }
                        }
                    }  else if (this.mapId == 216) {
                        ...
                        if (player.iDMark.getIndexMenu() == ConstNpc.BASE_MENU) {
                            if (select == 0) {
                                ChangeMapService.gI().changeMapBySpaceShip(player, 5, -1, 1319);
                            }
                        }
                    }else if (this.mapId == 5) {
                        if (player.iDMark.getIndexMenu() == ConstNpc.BASE_MENU) {
                        ...
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                case 0:
                                    ...
                                    break;
                                case 1:
                                    if (player.nPoint.power < 59999999999L) {
                                    Service.gI().sendThongBao(player, "Sức mạnh của bạn không đủ 60 tỷ!");
                                    return;
                                }
                                ...
                                case 1:
                                  if (player.nPoint.power < 59999999999L) {
                                    Service.gI().sendThongBao(player, "Sức mạnh của bạn không đủ 60 tỷ!");
                                        ...
                                        ChangeMapService.gI().changeMapBySpaceShip(player, 146, -1, 1112);
                                        break;
                                    }
                            }
                        }
        ...
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    ...
                    if (player.clanMember.getNumDateFromJoinTimeToToday() < 1 || (player.clan.doanhTrai != null && player.clanMember.getNumDateFromJoinTimeToToday() < 1)) {
                        createOtherMenu(player, ConstNpc.IGNORE_MENU,
                    ...
                    if (this.mapId == 24 || this.mapId == 25 || this.mapId == 26) {
                        try {
                            ...
                            if (now > BlackBallWar.TIME_OPEN && now < BlackBallWar.TIME_CLOSE) {
                                this.createOtherMenu(player, ConstNpc.MENU_OPEN_BDW, "Vào đại chiến ngọc rồng đen để chiến đấu, "
                                        + "Có mún vô không?",
                                        "Hướng dẫn\nthêm", "Start", "Từ chối");
                            } else {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (player.iDMark.isHoldBlackBall()) {
                        this.createOtherMenu(player, ConstNpc.MENU_PHU_HP, "Ta có thể giúp gì cho ngươi?", "Phù Để Mạnh Hơn", "Từ chối");
                    } else {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (!player.setClothes.huydietClothers) {
                        this.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Ngươi Hãy Mặc đủ 5 món Hủy Diệt",
                                "Đóng");
                    } else {
            ...
            if (canOpenNpc(player) && this.mapId == 46) {
                if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                    ...
                    if (player.clan != null && player.clan.ConDuongRanDoc != null && player.clan.gobosscdrd) {
                        ...
                        this.createOtherMenu(player, ConstNpc.BASE_MENU, message, "Không");
                    }
                            ...
                            switch (player.iDMark.getIndexMenu()) {
                                case ConstNpc.BASE_MENU:
                                    if (select == 0) {
                                        ...
                                        if (pudding != null && pudding.quantity >= 999
                                                || xucxich != null && xucxich.quantity >= 999
                                                || kemdau != null && kemdau.quantity >= 999
                                                || mily != null && mily.quantity >= 999
                                                || sushi != null && sushi.quantity >= 999) {
                                            ShopServiceNew.gI().opendShop(player, "BILL", true);
                                            break;
                                        } else {
                                            this.npcChat(player, "Còn không mau đem x999 thức ăn đến cho ta !!");
                                            break;
                                        }
                                    }
                            }
    ...
    public static Npc tapion(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (mapId == 19) {
                        this.createOtherMenu(player, 0, "Ngươi đã trên 40 tỷ có muốn săn đệ mabu không?", "Có", "Đéo");
                    }
                    if (mapId == 201) {
                        this.createOtherMenu(player, 0, "Mày Có Muốn Về Nhà Không?", "OK", "Từ chối");
                    ...
                    case ConstNpc.MENU_ADMIN:
                        ...
                        switch (select) {
                            ...
                            case 11:
                                ...
                                break;
                            case 12:
                                BossManager.gI().createBoss(BossType.zeno);
                                break;
                            case 13:
