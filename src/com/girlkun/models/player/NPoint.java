...
public class NPoint {
    ...
    private void setPointWhenWearClothes() {
        ...
        if (this.player.isPl() && this.player.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2) {
            ...
            for (Item item : this.player.inventory.itemsBag) {
                if (item.isNotNullItem() && item.template.id == 2129) {
                    for (Item.ItemOption io : item.itemOptions) {
                        switch (io.optionTemplate.id) {
                            case 14: //Chí mạng+#%
                                this.critAdd += io.param;
            ...
            for (Item item : this.player.inventory.itemsBag) {
                if (item.isNotNullItem() && item.template.id == 2130) {
                    for (Item.ItemOption io : item.itemOptions) {
                        switch (io.optionTemplate.id) {
                            case 14: //Chí mạng+#%
                                this.critAdd += io.param;
            ...
            for (Item item : this.player.inventory.itemsBag) {
                if (item.isNotNullItem() && item.template.id == 2131) {
                    for (Item.ItemOption io : item.itemOptions) {
                        switch (io.optionTemplate.id) {
                            case 14: //Chí mạng+#%
                                this.critAdd += io.param;
    ...
    private void setHpMax() {
        ...
        if (this.player.setClothes.nappa == 5) {
            this.hpMax += ((long) this.hpMax * 80 / 80);
        }
        //set worldcup
        ...
        if (this.player.zone != null && MapService.gI().isMapDiaNguc(this.player.zone.map.mapId)
                && !this.isKhongNong)
        {this.hpMax /= 2.5;
        }
        //mèo mun
    ...
    private void setMpMax() {
        ...
        if (this.player.effectSkill.isBienHinh) {
            if (!this.player.isPet || (this.player.isPet
                    && ((Pet) this.player).status != Pet.FUSION)) {
                int percent = SkillUtil.getPercentKiBienHinh(player.effectSkill.levelBienHinh);
                this.mpMax += ((long) this.mpMax * percent / 100);
            }
        }          
        //pet mabư
        if (this.player.isPet && ((Pet) this.player).typePet == 1
    ...
    private void setDame() {
        ...
        if (this.player.zone != null && MapService.gI().isMapDiaNguc(this.player.zone.map.mapId)&& !this.isKhongNong){this.dame /= 2.5;
        }
        //ngọc rồng đen 1 sao
        if (this.player.rewardBlackBall.timeOutOfDateReward[0] > System.currentTimeMillis()) {
        ...
        if (this.player.effectSkill.isBienHinh) {
            if (!this.player.isPet || (this.player.isPet
                    && ((Pet) this.player).status != Pet.FUSION)) {
                int percent = SkillUtil.getPercentDameMonkey(player.effectSkill.levelBienHinh);
...
//                }
        ...
        if (player.isPl() && dameAttack >= 50000000) {
                  ServerNotify.gI().notify(player.name + " đã đánh một chiêu " + player.playerSkill.skillSelect.template.name + " với sát thương " + Util.numberToMoney(dameAttack) +  " mọi người đều ngưỡng mộ");
                }
    ...
    public long calSubTNSM(long tiemNang) {
        if (this.player.zone != null && MapService.gI().isMapbang(this.player.zone.map.mapId)) {
            tiemNang /= 10000;
        }
