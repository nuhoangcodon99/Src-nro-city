...
public class Pet extends Player {
    ...
    private String getTextStatus(byte status) {
        switch (status) {
            case FOLLOW:
                return "để con đi theo sư phụ";
            case PROTECT:
                return "để con bảo vệ sư phụ";
            case ATTACK:
                return "để con tấn công chúng";
            case GOHOME:
                return "con về nhà đây";
            default:
                return "";
        }
    }
 ...
 public void fusion(boolean porata) {
        if (this.isDie()) {
            Service.getInstance().sendThongBao(master, "đệ chết rồi");
            return;
        }
        // if (this.inventory.itemsBody.get(5).isNullItem()) {
        //     Service.getInstance().sendThongBao(master, "yêu cầu đệ đeo cải trang");
        //     return;
        // }
    ...
    public void update() {
        try {
            ...
            if (isDie()) {
                if (System.currentTimeMillis() - lastTimeDie > 30000) {
...
//========================BETA SKILL5=====================
    ...
    private long lastTimeIncreasePoint;
     ...
     if (this.nPoint != null && Util.canDoWithTime(lastTimeIncreasePoint, 1)) {
            ...
            if (Util.isTrue(75, 100)) {
                this.nPoint.increasePoint((byte) 0, (short) 1);
            } else {
                byte type = (byte) Util.nextInt(0, 2);
                short point = (short) Util.nextInt(Manager.RATE_EXP_SERVER);
                this.nPoint.increasePoint(type, point);
            ...
            if (Util.isTrue(75, 100)) {
                this.nPoint.increasePoint((byte) 1, (short) 1);
            } else {
                byte type = (byte) Util.nextInt(0, 2);
                short point = (short) Util.nextInt(Manager.RATE_EXP_SERVER);
                this.nPoint.increasePoint(type, point);
