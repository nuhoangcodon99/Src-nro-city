...
public class SieuBoHung extends Boss {
   ...
   @Override
     public void reward(Player plKill) {
        ...
        byte randomNR = (byte) new Random().nextInt(Manager.itemIds_NR_SC.length);
        if (Util.isTrue(5, 100)) {
            Service.gI().dropItemMap(this.zone, Util.ratiItem(zone, Manager.itemIds_TL[randomDo], 1, this.location.x, this.location.y, plKill.id));
        } else {
            Service.gI().dropItemMap(this.zone, new ItemMap(zone, Manager.itemIds_NR_SC[randomNR], 1, this.location.x, zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id));
        }
